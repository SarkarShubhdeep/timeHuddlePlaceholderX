...
import { formatByteSize, pushSourceLabel } from "@/ui/lib/format";
...
function describeHealth(health) {
    ...
    if (h.healthy === false && h.reason === "api_error") {
        return typeof h.error === "string"
            ? h.error
            : "ActivityWork reported an error.";
    }
...
/**
 * Renders the three interactive cards for the Overview tab: latest snapshot
 * push summary, manual JSON import, and the optional "ActivityWork reachable
 * from this server" live toggle.
 */
        ...
        <div className="flex flex-col items-center gap-2">
                ...
                <CardHeader className="flex flex-row items-start justify-between gap-3 space-y-0">
                    <div className="space-y-1.5">
                        ...
                        <CardDescription>
                            ...
                            from activitywork-runtime, or{" "}
                            <span className="font-medium text-foreground">
                                Push
                            </span>{" "}
                            from aw-gateway). Stored in memory on this Meteor
                            process only. Full event list lives under the{" "}
                            <span className="font-medium text-foreground">
                                Pushed Snapshot
                            </span>{" "}
                            tab.
                    ...
                    typeof lastPushRecord.storedAt === "string" ? (
                        <div className="space-y-3">
                            ...
                            <dl className="grid gap-2 text-muted-foreground sm:grid-cols-2">
                                ...
                                {typeof lastPushRecord.bucketId === "string" ? (
            ...
            <Card className="w-full max-w-lg shadow-none">
                <CardHeader>
                    ...
                    <CardDescription>
                        Send a snapshot JSON from your machine (e.g. after you
                        confirm it in ActivityWork-runtime). This app does not
                        read ActivityWatch or your laptop&apos;s{" "}
                        <code className="rounded bg-muted px-1 py-0.5 text-xs">
                            localhost
                        </code>
                ...
                {activityWorkEnabled ? (
                    <CardContent className="space-y-3 border-t pt-4 text-sm">
                        <p className="text-muted-foreground">
                            ...
                            at your ActivityWork origin so the server can reach
                            it (default{" "}
                            <code className="rounded bg-muted px-1 py-0.5 text-xs">
                                http://localhost:5601
                            </code>
                                ...
                                activityWorkData.preview.ok === true ? (
                                    <p>
                                        ...
                                        {typeof activityWorkData.preview
                                            ...
                                            : "ok"}
                                        {activityWorkData.preview.latestEventAt
